                ...
                {activeTab === 'profile' && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="mt-6 pt-6 border-t border-gray-800">
                            <h3 className="text-sm font-bold text-white mb-4 uppercase tracking-widest">Appearance</h3>
                            <div className="flex bg-black/40 p-1.5 rounded-xl inline-flex border border-gray-700">
                                <button onClick={() => onSettingsChange({...settings, theme: 'light'})} className={`px-4 py-2 rounded-lg flex items-center gap-2 text-xs font-bold transition-all ${settings.theme === 'light' ? 'bg-white text-black' : 'text-gray-500'}`}><Sun size={14} /> Light</button>
                                <button onClick={() => onSettingsChange({...settings, theme: 'dark'})} className={`px-4 py-2 rounded-lg flex items-center gap-2 text-xs font-bold transition-all ${settings.theme === 'dark' ? 'bg-gray-800 text-white' : 'text-gray-500'}`}><Moon size={14} /> Dark</button>
                            </div>
                        </div>
                    </div>
                )}
...
const TabButton = ({ id, icon: Icon, label, active, set }: any) => (
    <button onClick={() => set(id)} className={`w-full flex items-center gap-3 px-6 py-4 rounded-xl transition-all ${active === id ? 'bg-primary text-black font-bold shadow-lg' : 'text-gray-500 hover:bg-gray-100 dark:hover:bg-white/5 dark:hover:text-white'}`}>
        <Icon size={18} /><span className="text-sm">{label}</span>{active === id && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-black dark:bg-white"></div>}
    </button>
);
...
const ToggleItem = ({ label, desc, icon: Icon, checked, onChange }: any) => (
    <div className="p-4 bg-gray-50 dark:bg-black/40 rounded-2xl border border-gray-200 dark:border-gray-700 flex items-center justify-between">
        ...
        <button onClick={() => onChange(!checked)} className={`w-12 h-7 rounded-full p-1 transition-colors ${checked ? 'bg-primary' : 'bg-gray-700'}`}><div className={`w-5 h-5 bg-white rounded-full shadow-md transform transition-transform ${checked ? 'translate-x-5' : 'translate-x-0'}`} /></button>
    </div>
);
