...
import { Camera, Upload, AlertCircle, RefreshCcw, Activity, Mic, Bot, Scan, Database, Zap, Layers, Scale, Crosshair, Eye, Info, ChevronRight, Target, Search, Dumbbell, Trophy, Zap as ZapIcon, Disc, Flag, Heart, Hexagon, Move, Flame, ArrowLeft, X, Play, AlertTriangle, Volume2, StopCircle, Youtube, ExternalLink, UserCheck, RotateCcw, ZoomIn, ZoomOut, Shield, Maximize2, Minimize2, Star, ThumbsUp, Hand, Lightning } from 'lucide-react';
    ...
    return (
        <div className="absolute bottom-6 left-6 z-50 flex items-end gap-3 pointer-events-none transition-all duration-500">
             <div className="relative w-24 h-24 flex items-center justify-center">
                 ...
                 <div className="relative z-10 w-16 h-16 bg-black/90 border border-white/10 rounded-full flex items-center justify-center overflow-hidden backdrop-blur-md shadow-[0_0_30px_rgba(0,0,0,0.5)]">
                     <div className="flex items-center gap-[3px] h-8">
                        {[...Array(5)].map((_, i) => (
                            <div 
                                key={i}
                                className={`w-1.5 rounded-full transition-all duration-75 shadow-[0_0_10px_currentColor] ${state === 'listening' ? 'bg-accent' : state === 'processing' ? 'bg-yellow-400' : emotion === 'happy' ? 'bg-green-400' : 'bg-primary'}`}
                                style={{ 
                 ...
                 <div className={`absolute top-1 right-1 w-3.5 h-3.5 rounded-full border-2 border-black ${state !== 'idle' ? 'bg-green-400 shadow-[0_0_10px_#4ade80]' : 'bg-gray-600'}`}></div>
             </div>
             ...
             <div className={`mb-6 transition-all duration-500 transform ${state !== 'idle' ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'}`}>
                 <div className="bg-black/80 backdrop-blur-xl border-l-2 border-primary px-4 py-2 rounded-r-xl shadow-lg">
                     ...
                     <span className="text-sm text-white font-black uppercase tracking-wider flex items-center gap-2">
                        ...
                        {state === 'processing' && <span className="text-[9px] text-yellow-400 animate-pulse">• THINKING</span>}
                     </span>
                 </div>
             </div>
        </div>
...
const SkeletonVisualizer: React.FC<{ result: AnalysisResult, showHeatmap: boolean }> = ({ result, showHeatmap }) => {
    ...
    const getColor = (status: string, stress: number) => {
        ...
        if (showHeatmap && stress > 40) return '#f97316'; // Orange
        switch(status) {
            case 'critical': return '#ef4444'; 
            case 'warning': return '#eab308'; 
            case 'optimal': return '#06b6d4'; 
                ...
                {connections.map(([start, end], i) => {
                    ...
                    return (
                        <line 
                            ...
                            x1={s.x} y1={s.y}
                            x2={e.x} y2={e.y}
                            stroke={showHeatmap ? 'rgba(255, 255, 255, 0.2)' : 'rgba(6, 182, 212, 0.3)'}
                            strokeWidth="2"
                            strokeLinecap="round"
                        />
                    );
