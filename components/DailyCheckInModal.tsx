...
const DailyCheckInModal: React.FC<DailyCheckInModalProps> = ({ onComplete, username }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/95 backdrop-blur-xl animate-in fade-in duration-500">
            <div className="w-full max-w-lg bg-[#0a0a0b] border border-gray-800 rounded-[32px] overflow-hidden shadow-2xl relative">
                ...
                <div className="p-8 md:p-12 flex flex-col items-center text-center">
                    ...
                    {step === 1 && (
                        <div className="space-y-8 w-full animate-in slide-in-from-right fade-in duration-300">
                            <div>
                                <h2 className="text-3xl font-black text-white italic tracking-tighter mb-2">GOOD MORNING, {username.toUpperCase()}</h2>
                                <p className="text-gray-400">Let's calibrate your system for today.</p>
                            </div>

                            <div className="bg-surface border border-gray-800 p-6 rounded-2xl w-full">
                                <div className="flex justify-between items-center mb-4">
                                    <span className="font-bold text-white flex items-center gap-2"><Moon className="text-primary" size={20}/> Hours Slept</span>
                            ...
                            <div className="bg-surface border border-gray-800 p-6 rounded-2xl w-full">
                                <div className="flex justify-between items-center mb-4">
                                    <span className="font-bold text-white flex items-center gap-2"><Battery className="text-yellow-400" size={20}/> Energy Level</span>
                                    <span className="text-2xl font-black text-yellow-400">{energy}/10</span>
                                </div>
                                <input 
                    ...
                    {step === 2 && (
                        <div className="space-y-8 w-full animate-in slide-in-from-right fade-in duration-300">
                            <div>
                                <h2 className="text-3xl font-black text-white italic tracking-tighter mb-2">BODY STATUS</h2>
                                <p className="text-gray-400">Any pain or stiffness preventing peak performance?</p>
                            </div>

                            <div className="bg-surface border border-gray-800 p-6 rounded-2xl w-full">
                                <div className="flex justify-between items-center mb-4">
                                    <span className="font-bold text-white flex items-center gap-2"><AlertCircle className="text-red-500" size={20}/> Soreness Level</span>
                    ...
                    {step === 3 && (
                        <div className="space-y-8 w-full animate-in slide-in-from-right fade-in duration-300">
                            <div>
                                <h2 className="text-3xl font-black text-white italic tracking-tighter mb-2">MINDSET</h2>
                                <p className="text-gray-400">How are you feeling mentally today?</p>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                {['Great', 'Good', 'Okay', 'Tired', 'Stressed'].map((m) => (
                                    <button 
                                        ...
                                        className={`p-4 rounded-xl border font-bold transition-all ${mood === m ? 'bg-primary text-black border-primary' : 'bg-surface border-gray-700 text-gray-400 hover:border-gray-500'}`}
                                    >
                                        {m}
                                    </button>
                                ))}
                    ...
                    <button 
                        onClick={handleNext}
                        className="mt-8 w-full py-4 bg-white text-black font-black rounded-xl text-lg uppercase tracking-widest hover:bg-gray-200 transition-all flex items-center justify-center gap-2 group"
                    >
